...
class TestFeedbackForms:
    ...
    def verify_successful_feedback_submission(self):
        feedback_success_heading = WebDriverWait(self.selenium, 10).until(
            EC.presence_of_element_located((By.XPATH, '//h2[text()="Thank you for your feedback"]'))
        )
    ...
    def verify_invalid_feedback_submission_shows_errors(self):
        error_summary = WebDriverWait(self.selenium, 10).until(
            EC.presence_of_element_located((By.CLASS_NAME, "govuk-error-summary__title"))
        )
    ...
    def verify_no_feedback_form_rendered(self):
        not_clear = WebDriverWait(self.selenium, 10).until(EC.presence_of_element_located((By.ID, "id_not_clear")))
        assert not_clear is not None
        assert "Can you tell us more?" in self.selenium.page_source

    def verify_problem_feedback_form_rendered(self):
        not_working = WebDriverWait(self.selenium, 10).until(EC.presence_of_element_located((By.ID, "id_not_working")))
    ...
    def populate_yes_feedback_form(self, invalid=False):
        if not invalid:
            ...
            information_useful = self.selenium.find_element(By.ID, "id_information_useful")
            information_useful.click()

            information_easy_to_understand = self.selenium.find_element(By.ID, "id_information_easy_to_understand")
    ...
    def populate_no_feedback_form(self, invalid=False):
        if not invalid:
            ...
            information_not_avaialable = self.selenium.find_element(By.ID, "id_information_not_available")
            information_not_avaialable.click()

            incomplete_information = self.selenium.find_element(By.ID, "id_incomplete_information")
            incomplete_information.click()

            difficult_to_understand = self.selenium.find_element(By.ID, "id_difficult_to_understand")
